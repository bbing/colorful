...
class Upload extends Admin_Controller
{
	...
	public function index()
	{
		...
		$attachDir= dirname(BASEPATH) . '/images/upload';//上传文件保存路径，结尾不要带/
		...
		if($err==''){
			...
			if(preg_match('/^('.str_replace(',','|',$upExt).')$/i',$extension))
			{
				...
				else
				{
					...
					$fileurl = self::jsonString("/images/upload/$attachSubDir/$newFilename");
					if($immediate=='1')$targetPath='!'.$targetPath;
					if($msgType==1)$msg="'$targetPath'";
					else $msg="{'url':'".$fileurl ."','localname':'".self::jsonString($localName)."','id':'1'}";//id参数固定不变，仅供演示，实际项目中可以是数据库ID
				}
			}
